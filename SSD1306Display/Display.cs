...
namespace SSD1306Display
{
    ...
    class Display
    {
        ...
        public Display()
        {
            ...
            {
                byte[] line = new byte[SCREEN_HEIGHT_PAGES];
   ...
   //             throw;
        ...
        public bool IsDeviceConnected()
        {
            ...
            {
                byte[] testbyte = { 0 };
                I2cTransferResult result = displayI2c.WritePartial(testbyte);
                ...
                if (result.Status == I2cTransferStatus.ClockStretchTimeout)
                {
                    return false;
                }

            //    this.displayI2c.Write(testbyte);
                return true;
            }
            catch (Exception ex)
            {
        ...
        /* Send I2C commands to power up and initialize the display */
        public bool InitDisplay()
        {
            /* Initialize the display */
            try
            {
                byte[] reset = { 0xe4 };
                DisplaySendCommand(reset);  /* Reset          */
        ...
        public bool DisplayUpdate()
        {
            ...
            for (int PageY = 0; PageY < SCREEN_HEIGHT_PAGES; PageY++)
            {
                ...
                {
                    byte[] Page = DisplayBuffer[PixelX];
                    byte Pixel = Page[PageY];
        ...
        {           
            ...
            for (CurrentCol = StartCol; CurrentCol < EndCol; CurrentCol++)                              
            {
                for (CurrentPage = StartPage; CurrentPage < EndPage; CurrentPage++)
                {
                    ...
                    byte[] Page = DisplayBuffer[(int)CurrentCol];
                    //       byte Pixel = CharDescriptor.CharacterData[CharDataIndex];
                    Page[CurrentPage] = img.ImageData[CharDataIndex];
